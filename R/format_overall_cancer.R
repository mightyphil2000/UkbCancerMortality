#' Format overall cancer mortality data 
#'
#' Format the overall cancer mortality dataset so that it can be used in survival analyses. The 
#' function estimates the number of deaths due to any cancer and other causes, date of diagnosis, date of death, difference between latest and earlies dates of diagnosis for individuals with more than one diagnosis date, length of followup and survival time.  
#'
...
format_overall_cancer<-function(dat=NULL,censor_date="2018-02-06"){
	...
	dat<-number_deaths_overall_cancer(dat=dat)
...
number_deaths_overall_cancer<-function(dat=NULL){
	...
	Posdiff<-which(dat$f.40001.1.0[Pos1] != dat$f.40001.0.0[Pos1]  )
	if(!all(dat$f.40001.1.0[Pos1] == dat$f.40001.0.0[Pos1]  )) warning(paste0("second instance of f.40001 primary cause of death is different to the first instance: for ",length(Posdiff)," people [",paste0(dat[Pos1[Posdiff],c("f.40001.1.0","f.40001.0.0")],collapse="; "),"]"))
		
	dat$primary<-NA
	dat$primary[grep("C",dat$f.40001.0.0)]<-"Malignant neoplasm"
